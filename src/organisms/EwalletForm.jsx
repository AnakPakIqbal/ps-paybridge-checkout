...
import WalletOption from '../molecules/WalletOption.jsx'
...
export default function EwalletForm({
  ...
  if (paymentAttempt) {
    ...
    const isQris = methodLower.includes('qris') || methodLower.includes('qr_code') || methodLower.includes('qr')
    
    // Determine if we should generate a QR code image
    const qrUrl = isQris && paymentAttempt.checkoutUrl
      ? (paymentAttempt.checkoutUrl.match(/\.(png|jpg|jpeg|gif)/i)
        ...
        {qrUrl ? (
          <div className="flex flex-col items-center max-w-sm">
            ...
            <div className="bg-white p-4.5 rounded-xl2 shadow-xl border border-white/10 mb-5 relative group overflow-hidden">
              ...
              <img src={qrUrl} alt="QRIS QR Code" className="w-48 h-48 sm:w-52 sm:h-52 relative z-10" />
            </div>
            ...
            <div className="text-left w-full text-xs text-muted leading-relaxed space-y-3.5 bg-panel px-4 py-4 rounded-xl border border-lineSoft">
              <p className="font-semibold text-text text-center border-b border-lineSoft pb-1.5 mb-2.5">How to Pay</p>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <span>Choose <span className="text-text font-medium">Scan QR / Pay</span> and upload/scan this QR code.</span>
              </div>
              <div className="flex items-start gap-2.5">
          ...
          <div className="flex flex-col items-center max-w-sm">
            ...
            <p className="text-xs text-muted mb-6">
              Confirm your transaction in the external application. You will be redirected shortly or click the button below.
            </p>
            {paymentAttempt.checkoutUrl && (
              <a
                ...
                className="w-full flex items-center justify-center gap-2 bg-brand text-black font-semibold py-3.5 px-5 rounded-xl hover:bg-brand/90 transition-all duration-150 shadow-lg shadow-brand/10"
              >
                Open E-Wallet App
  ...
  const walletMethods = availableMethods.filter(m => m.category === 'e_wallet' || m.category === 'qr_code')
